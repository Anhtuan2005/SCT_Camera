...
ROOT = Path(r"E:\SCT_Camera\tmp\line_negative_review_20260819_2")
...
def main() -> None:
    ...
    for video_path in sorted(ROOT.glob("*.mp4")):
        ...
        tiles = []
        brightness = []
        sample_times = np.linspace(0.0, max(duration_s - 0.05, 0.0), 20)
        ...
        rows = [cv2.hconcat(tiles[start : start + 5]) for start in range(0, 20, 5)]
        sheet_path = ROOT / f"contact_{video_path.stem}.jpg"
